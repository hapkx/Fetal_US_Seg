...
# Visdom 可视化设置，初始化 Visdom 实例，用于实时监控训练过程中的损失和梯度变化。
...
# 设置初始的日志损失缩放值，通常在训练开始时用作默认值。
# For ImageNet experiments, this was a good default value.
# We found that the lg_loss_scale quickly climbed to
# 20-21 within the first ~1K steps of training.
INITIAL_LOG_LOSS_SCALE = 20.0

# 归一化图像函数，定义一个函数用于归一化图像，以便在可视化时保持图像的对比度。
def visualize(img):
    _min = img.min()
    _max = img.max()
...
class TrainLoop:
    def __init__(
        ...
        *,
        ...
        use_fp16=False,     # 是否使用半精度浮点数（FP16）
        fp16_scale_growth=1e-3,     # FP16缩放增长率
        schedule_sampler=None,  # 采样器
    ...
    def forward_backward(self, batch, cond):
        ...
        for i in range(0, batch.shape[0], self.microbatch):
            ...
            # 检查当前处理的微批次是否为最后一批。这有助于决定是否需要同步模型的状态。
            last_batch = (i + self.microbatch) >= batch.shape[0]
            # 使用 self.schedule_sampler 采样时间步和相应的权重。这些时间步用于计算损失。
            ...
            # 根据是否为最后一批以及是否使用分布式数据并行（DDP）来决定如何计算损失：
            # 如果是最后一批或不使用 DDP，则直接计算损失。
            # 如果不是最后一批且使用 DDP，则在不同步的上下文中计算损失，以减少GPU之间的通信开销。
            ...
            # 如果使用的是 LossAwareSampler，则更新采样器以反映当前微批次的局部损失。
