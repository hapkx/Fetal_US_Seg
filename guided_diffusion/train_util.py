...
class TrainLoop:
    ...
    def run_loop(self):
        ...
        while (
            ...
            logger.log(f"=========================run loop: {i}=========================")

            start_i = th.cuda.Event(enable_timing=True)
            end_i = th.cuda.Event(enable_timing=True)
            ...
            # 
            # cond = cond.permute(0, 3, 1, 2).to(config["device"])  # [B, C, H, W]
            # 
            # batch  cond: 1, 3, 256,256
            ...
            end_i.record()
            th.cuda.synchronize()
            print('time for 1 loop: ', start_i.elapsed_time(end_i))  #time measurement for the generation of 1 sample
    ...
    def save(self):
        def save_checkpoint(rate, params):
            ...
            if dist.get_rank() == 0:
                # 记录日志：正在保存模型（rate为0表示主模型，非0表示EMA模型）
                logger.log(f"saving model {rate}...")
                if not rate:
                    # 主模型文件名格式：savedmodel{step}.pt
                    filename = f"savedmodel{(self.step+self.resume_step):06d}.pt"
                else:
                    # EMA模型文件名格式：emasavedmodel_{rate}_{step}.pt
...
# 从文件名解析训练步数（如 model001000.pt → 1000）
...
def log_loss_dict(diffusion, ts, losses):
    for key, values in losses.items():
        # key 是损失的名称（如 "mse_loss"），values 是对应的损失值张量
        logger.logkv_mean(key, values.mean().item())
        # Log the quantiles (four quartiles, in particular).
        for sub_t, sub_loss in zip(ts.cpu().numpy(), values.detach().cpu().numpy()):
            # 将 ts 和 values 转换为 NumPy 数组，并通过 zip 将它们配对进行迭代。
