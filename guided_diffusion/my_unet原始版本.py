...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps, y=None):
        ...
        # print("y: ", y)
        # x.shape:  torch.Size([4, 4, 256, 256])
        ...
        pvt_f = self.pvt(c)     
        # print("len pvt_f: ", len(pvt_f))  4
        # pvt_f[0].shape [4, 64, 256, 256]
        # pvt_f[1].shape [4, 128, 128, 128]
        # pvt_f[2].shape [4, 256, 64, 64]
        # pvt_f[3].shape [4, 512, 32, 32]
        # h.shape [4, 4, 256, 256]
        
        for ind, module in enumerate(self.input_blocks):
            if len(emb.size()) > 2:
                emb = emb.squeeze() # torch.Size([4, 256])
                
            if ind%3==2:
                with torch.no_grad():  # 阻断梯度回传
                    # 1111
                    pvt_feat = pvt_f[ind//3].detach()  # 分离计算图
                h = module(self._fuse(h, pvt_feat), emb)  # 改进融合函数
            else:
                h = module(h, emb)
            hs.append(h)
...
#     def __init__(self, input_channels, base_num_features, num_classes, num_pool, pvt):
...
#             in_channels = input_channels if i == 0 else base_num_features * (2**(i))
#             out_channels = base_num_features * (2**(i + 1))
...
#         for i in range(num_pool):
#             in_channels = base_num_features * (2**(num_pool - i))
#             out_channels = base_num_features * (2**(num_pool - i - 1))
...
#         # 输出层
#         self.out = nn.Conv2d(base_num_features, num_classes, kernel_size=1)
...
#             x = encoder_block(x)
#             if i < len(pvt_features):
#                 pvt_feat = pvt_features[i]
#                 # 特征融合，这里采用拼接的方式，你也可以用其他方式
#                 x = torch.cat([x, pvt_feat], dim=1)
#             skips.append(x)
