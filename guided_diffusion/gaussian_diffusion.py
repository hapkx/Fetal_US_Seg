...
def get_named_beta_schedule(schedule_name, num_diffusion_timesteps):
    ...
    定义噪声调度策略，控制扩散过程中每一步的噪声水平β_t
    线性调度：β_t从0.0001到0.02线性增长
    余弦调度：根据α_bar的余弦衰减设计β序列
    """
...
def betas_for_alpha_bar(num_diffusion_timesteps, alpha_bar, max_beta=0.999):
    ...
    数学原理：根据α_bar(t) = Π(1-β_s)计算β序列
    保证β_t满足：1 - β_t = α_bar(t)/α_bar(t-1)
    """
    betas = []
...
class ModelMeanType(enum.Enum):
    ...
    """
    # enum.auto() 是 enum 模块中的一个实用函数，用于自动为枚举成员生成唯一的值。用于定义模型的不同配置选项
...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        alphas = 1.0 - betas
        self.alphas_cumprod = np.cumprod(alphas, axis=0)        # α_bar_t = Π(1-β_s)
    ...
    def q_sample(self, x_start, t, noise=None):
        ...
        :param x_start: the initial data batch. 原始输入数据（未加噪），形状为 (B, C, H, W)
        :param t: the number of diffusion steps (minus 1). Here, 0 means one step.
    ...
    def q_posterior_mean_variance(self, x_start, x_t, t):
        ...
        Compute the mean and variance of the diffusion posterior:
            q(x_{t-1} | x_t, x_0)
        计算反向过程的后验分布q(x_{t-1}|x_t,x_0)
        均值公式：μ̃ = [β_t*sqrt(α_bar_{t-1})x_0 + (1-α_bar_{t-1})sqrt(α_t)x_t]/(1-α_bar_t)
        """
    ...
    def p_mean_variance(
        ...
        x=x[:,-3:,...]  #loss is only calculated on the last channel, not on the input brain MR image
    ...
    def p_sample(
        ...
        """
        # 调用 p_mean_variance 函数，该函数会应用模型得到 p(x_{t-1} | x_t) 的均值、方差、对数方差以及对初始 x_0 的预测
        ...
        noise = th.randn_like(x[:, -3:,...])
        # 创建一个非零掩码，当 t 不为 0 时为 1，否则为 0，用于确保在 t = 0 时不添加噪声
        nonzero_mask = (
            (t != 0).float().view(-1, *([1] * (len(x.shape) - 1)))
        )
        # 根据采样公式计算样本：sample = 均值 + 非零掩码 * 指数(0.5 * 对数方差) * 噪声
    ...
    def p_sample_loop_known(
        ...
        img = img.to(device)
        noise = th.randn_like(img[:, :3, ...]).to(device)
        x_noisy = torch.cat((img[:, :-3,  ...], noise), dim=1)  #add noise as the last channel
        img=img.to(device)
    ...
    def p_sample_loop_progressive(
        ...
        org_c = img.size(1)
        org_MRI = img[:, :-3, ...]      #original brain MR image
        if progress:
    ...
    def training_losses_segmentation(self, model, classifier, x_start, t, model_kwargs=None, noise=None):
        ...
        if model_kwargs is None:
            model_kwargs = {}
        if noise is None:
            noise = th.randn_like(x_start[:, -3:, ...])


        mask = x_start[:, -3:, ...]     # 提取多类分割标签
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.RESCALED_MSE:
            ...
            # b1,c1,h1,w1 = model_output.shape
            # b2,c2,h2,w2 = target.shape
            # model_output, target = model_output.reshape(b1,c1,-1), target.reshape(b2,c2,-1)
            # print("model output shape:")
            # print(model_output.shape)
            # print("target shape:")
            # print(target.shape)

            # # 检查 model_output 是否为 logits，且不包含非法值
            # print("Min logits:", model_output.min().item(), "Max logits:", model_output.max().item())
            # assert not torch.any(torch.isnan(model_output)), "Model output contains NaN values"

            # # 检查 target 的值范围
            # print("Target min:", target.min().item(), "Target max:", target.max().item())
            # assert target.dtype == torch.long, "Target must be integer indices"
            # 

            ce_loss = F.cross_entropy(model_output, target, reduction='none')
            # print("ce_loss: ", ce_loss)
            terms["loss_diff"] = mean_flat(ce_loss)
            # terms["mse"] = mean_flat((target - model_output) ** 2)
            # print("terms[mse]: ", terms["mse"])
            # 添加Dice损失
            dice_loss = 1 - dice_score(F.softmax(model_output, dim=1), target)
            # print("dice_loss: ", dice_loss)
            terms["loss_diff"] += dice_loss
            # total_loss = 0.5*dice_loss + 0.5*ce_loss
            # terms["loss_diff"] = mean_flat(total_loss)
            # terms["loss_diff"] = total_loss

            # if "vb" in terms:
            #     terms["loss"] = terms["mse"] + terms["vb"]+ terms["loss_diff"]
            # else:
            #     terms["loss"] = terms["mse"]+ terms["loss_diff"]
            if "vb" in terms:
