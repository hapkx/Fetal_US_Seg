...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        self.pvt = self.load_pretrained_pvt_b0(pvt)
    ...
    # 加载预训练的PVTv2 - B0模型
    def load_pretrained_pvt_b0(self, pvt_model):
        try:
            # 从timm库中加载PVTv2 - B0的预训练权重
            from timm.models import pvt_v2_b0
            pretrained_model = pvt_v2_b0(pretrained=True)
            pretrained_state_dict = pretrained_model.state_dict()

            # 获取当前模型的状态字典
            model_state_dict = pvt_model.state_dict()
            ...
            # 加载更新后的状态字典
            pvt_model.load_state_dict(model_state_dict)
            print("成功加载预训练的PVTv2 - B0权重。")
