...
class EncoderUNetModel(nn.Module):
    ...
    def __init__(
        ...
        # 定义时间嵌入层，映射时间步嵌入到更高维度，并通过 SiLU 激活函数处理。
        time_embed_dim = model_channels * 4
        self.time_embed = nn.Sequential(
            linear(model_channels, time_embed_dim),
            nn.SiLU(),
        ...
        # 初始化输入块，存储在 ModuleList 中，用于构建模型的层次结构。
        # 初始化一些变量，用于跟踪特征通道数和降采样倍率。
        self.input_blocks = nn.ModuleList(
            [
                TimestepEmbedSequential(
                    conv_nd(dims, in_channels, model_channels, 3, padding=1)
        ...
        # 循环构建每一层的 ResBlock（残差块），根据 channel_mult 和 num_res_blocks 进行配置。
        for level, mult in enumerate(channel_mult):
            for _ in range(num_res_blocks):
                layers = [
                    ResBlock(
                ...
                ch = mult * model_channels
                # 如果当前降采样倍率 ds 在注意力分辨率列表中，则添加注意力块。
                if ds in attention_resolutions:
                    layers.append(
                        AttentionBlock(
                            ch,
                ...
                # 将构建的块（包括残差块和可能的注意力块）添加到输入块中。
                # 更新特征尺寸和通道数信息。
                self.input_blocks.append(TimestepEmbedSequential(*layers))
                self._feature_size += ch
                input_block_chans.append(ch)

            # 如果当前不是最后一层，则添加降采样块，可能通过残差块或降采样层实现。
            if level != len(channel_mult) - 1:
                out_ch = ch
                self.input_blocks.append(
                    TimestepEmbedSequential(
        ...
        # 定义中间块（瓶颈层），通常由一个残差块、一个注意力块和另一个残差块组成，用于在最小特征图尺寸上提取特征。
        self.middle_block = TimestepEmbedSequential(
            ResBlock(
                ch,
                time_embed_dim,
